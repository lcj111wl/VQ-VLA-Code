...
def cosine_scheduler(base_value, final_value, total_steps, warmup_steps=0, start_warmup_value=0):
    ...
    Args:
        base_value (float): Maximum learning rate value.
        final_value (float): Minimum learning rate value.
        total_steps (int): Total number of training steps.
        warmup_steps (int): Number of warmup steps. Default: 0.
        start_warmup_value (float): Initial learning rate during warmup. Default: 0.

    Returns:
        np.ndarray: Learning rate schedule for each training step.
    """
    # Warmup phase
    warmup_schedule = np.array([])
    if warmup_steps > 0:
    ...
    # Ensure the schedule length matches total_steps
    assert len(schedule) == total_steps
    return schedule


def constant_scheduler(base_value, epochs, niter_per_ep, warmup_epochs=0, start_warmup_value=1e-6, warmup_steps=-1):
    warmup_schedule = np.array([])
    warmup_iters = warmup_epochs * niter_per_ep
    if warmup_steps > 0:
        warmup_iters = warmup_steps
...
def get_parameter_groups(
    model, weight_decay=1e-5, base_lr=1e-4, skip_list=(), get_num_layer=None, get_layer_scale=None, **kwargs
):
...
def create_optimizer(
    args, model, get_num_layer=None, get_layer_scale=None, filter_bias_and_bn=True, skip_list=None, **kwargs
):
    ...
    if "fused" in opt_lower:
        assert torch.cuda.is_available(), "APEX and CUDA required for fused optimizers"
...
def save_model(args, step, model, model_without_ddp, optimizer, loss_scaler, model_ema=None, save_ckpt_freq=1):
    output_dir = Path(args.output_dir)
    step_name = str(step)

    checkpoint_paths = [output_dir / "checkpoint.pth"]

    checkpoint_paths.append(output_dir / ("checkpoint-%s.pth" % step_name))
...
class NativeScalerWithGradNormCount:
    ...
    def __call__(
        self, loss, optimizer, clip_grad=None, parameters=None, create_graph=False, update_grad=True, layer_names=None
    ):
