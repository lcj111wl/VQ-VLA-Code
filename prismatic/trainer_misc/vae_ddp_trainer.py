...
def train_action_vqvae(
    ...
    if is_main_process():
        wandb.init(project="VQ-VLA", name=args.wandb_name)
    ...
    for step in metric_logger.log_every(range(initial_global_step, total_steps), print_freq):
        ...
        with torch.cuda.amp.autocast(enabled=True, dtype=_dtype):
            act = batch["actions"]
            commit_loss, rec_loss, total_loss, loss_breakdown = model(act)

        loss_log = {k: v.item() for k, v in loss_breakdown.items()}
        ...
        if is_main_process():
            log_dict = {f"train/{k}": v for k, v in loss_log.items()}
            log_dict["train/total_loss"] = loss_value.item()
            log_dict["train/lr"] = max_lr
            log_dict["train/min_lr"] = min_lr
            log_dict["train/weight_decay"] = weight_decay_value
            log_dict["train/grad_norm"] = grad_norm
            log_dict["train/loss_scale"] = loss_scale_value
            wandb.log(log_dict, step=initial_global_step)
